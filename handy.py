...
def unpickle(run='02', sensor_group='all', loc='', start='RUN', end='_100Hz_cut', time=True, runbyindex=False,
             column_only=False, n_seg=1, seg_nr=0, reset_time=False, pos_pad=0, neg_pad=0):
    ...
    if reset_time: df.timestamp = np.linspace(0, (df.index[-1]) * (df.timestamp[1] - df.timestamp[0]),
                                              num=df.index[-1] + 1, endpoint=True)
    return df
...
def getFeatureList(n_splits, glob=False):
    ...
    if glob == False:
        df = pd.read_pickle('n_splits_%i_sensor_AL02.pkl' % (n_splits))
    else:
...
def combineSensors(sensors, n_splits, wantedFeatures):
    ...
    df = pd.DataFrame()
    damage = pd.read_pickle('n_splits_' + str(n_splits) + '_sensor_' + sensors[0] + '.pkl')[
        'Damage']
    for sensorN in sensors:
        temp_df = pd.read_pickle('n_splits_' + str(n_splits) + '_sensor_' + sensorN + '.pkl')
